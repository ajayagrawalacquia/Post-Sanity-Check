...
# A quick overview of the tool here -> https://github.com/acquia/ops-misc/blob/master/TOOLS.md
#
################################################################################
...
check_high_load_by_pct() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$load" =~ ^[0-9.]+$ ]]; then
            local rounded_load=$(printf "%.2f" "$load")
            if (( $(echo "$rounded_load > 70" | bc -l) )); then
                echo -e "$line"
            fi
        fi
    done <<< "$webloads"
...
[ $# -lt 1 ] && echo "Usage: ahops-sanity-safeguard Site/Server" && exit 1;
